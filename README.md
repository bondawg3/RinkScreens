...
### Display scheduling
Each display can be preset with what it shows through the day, up to a month ahead (📅 button on the display card opens a full-page **visual scheduler**). Add a screen to the timeline three ways: drag a thumbnail from the type-grouped palette, click an empty time slot and then click a screen, or right-click a thumbnail → "Add to schedule". Blocks snap to 15-minute boundaries, default to one hour, and resize by dragging their edges. Dropping onto an occupied time shifts neighbouring blocks to the nearest free space (never shrinking them), or prompts to replace when the day is full. Each block has remove and whole-day buttons.

Switch between **Day**, **3-day**, and **Week** views; the timeline shows the whole 12 AM–12 AM day, with a Fit / Comfortable / Large zoom. In the multi-day views a block's ⧉ handle drags it onto another day to copy it. Move between displays from the scheduler header (‹ › and a dropdown) without leaving the page. **Duplicate day / week** opens a dialog to pick the destination (tomorrow, next week, or a specific date/week) and choose Skip or Replace for each destination day that already has overlapping blocks. The palette's "All screens" checkbox shows or hides every type at once.

Outside any block the display falls back to its assigned screen, and TVs switch exactly at block boundaries. The last 30 days of schedule history are kept (older blocks pruned daily). View preferences (view, zoom, side-swap, column density, week start, hidden types) are saved server-side for all admins.

...
export default function SkateTab() {
  ...
  const [form, setForm] = useState({ label: '', price: '', sort_order: '' });
  ...
  async function addPrice(e) {
    ...
    setForm({ label: '', price: '', sort_order: '' });
    reload();
  }
  ...
  return (
    <div>
      ...
      <table className={styles.table}>
        <thead>
          <tr><th>Label</th><th>Price</th><th>Sort</th><th></th></tr>
        </thead>
        <tbody>
          {(prices || []).map((p) => (
            <tr key={p.id}>
              {editing === p.id ? (
                ...
                <>
                  ...
                  <td>{p.sort_order}</td>
                  <td className={styles.actions}>
                    <button className={styles.btnGhost} onClick={() => { setEditing(p.id); setEditData({ label: p.label, price: p.price, sort_order: p.sort_order }); }}>Edit</button>
          ...
          {prices && prices.length === 0 && <tr><td colSpan={4} className={styles.muted}>No prices yet.</td></tr>}
        </tbody>
      </table>
    </div>
  );
